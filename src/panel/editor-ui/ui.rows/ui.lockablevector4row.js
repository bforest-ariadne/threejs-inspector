UI.LockableVector4Row = function () {
	var row	= this
	UI.Panel.call( this );
  ...
  var valueW	= new UI.Number().setWidth( '50px' ).setColor('black').onChange( function(){
		changeSrc	= 'fromW'
		dispatchChange()
		changeSrc	= ''
	} );
  ...
  this.valueW	= valueW
	...
	container.add( valueX, valueY, valueZ, valueW );
	...
	this.update	= function(vector){
		...
		if( row.isLocked() === true ){
			if( changeSrc === 'fromX' ){
				...
				valueY.setValue( valueY.getValue() * ratio );
        valueX.setValue( valueX.getValue() * ratio );
      }else if( changeSrc === 'fromW' ){
				var ratio	= valueW.getValue() / vector.z;
				valueY.setValue( valueY.getValue() * ratio );
				valueX.setValue( valueX.getValue() * ratio );				
			}else if( changeSrc === '' ){
    ...
    valueW.setValue( vector.w );
	}
}

UI.LockableVector4Row.prototype = Object.create( UI.Panel.prototype );
